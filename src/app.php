...
function summary($records, "duration") {
  $count = count($records);
  if ("duration" === null) return "count=$count";
  $total = 0;
  foreach ($records as $r) { $total += intval($r["duration"] ?? 0); }
  return "count=$count, {"duration"}_total=$total";
}
